...
@author: schaefej51
openmm simulation script for a unbiased simulation at OpenMM 4 states potential custom force 
"""
import numpy as np
import argparse

from reweightingtools.potential.fourStatesScale import fourStates
...
kB=0.008314
collision_rate=1*(picoseconds)**(-1)
...
startingPositions= np.array([[-0.5,0.5,0.5]]) + 0.1 * (np.random.rand(nParticles, 3)) 

analyticforce = fourStates(1,10)
with open("OPESx/onX/plumed.dat","r") as f:
    plumedScript = f.read()
perturbation = PlumedForce(plumedScript)
